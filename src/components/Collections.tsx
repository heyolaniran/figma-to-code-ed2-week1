      ...
      <div className="mt-2 px-2 lg:flex md:flex lg:justify-between md:justify-between items-center">
        <div className=" px-2 flex  overflow-x-auto hide-scrollbar">
          {categories.map((category) => (
            <button
              key={category.id}
              className={` ${category.active ? "bg-black text-gray-100" : "bg-gray-200 text-black"} text-xs px-4 py-1 mx-2 rounded-md `}
            >
              {category.name}
            </button>
          ))}
        ...
        <Link
          href={""}
          className=" underline-offset-1 underline text-xs font-semibold"
        >
          {" "}
          view more{" "}
        </Link>
      ...
      <div className=" grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-6 mt-2 px-2">
        {collections.map((collection) => (
          <div className="border border-spacing-1 my-2 p-6 rounded-2xl">
            ...
            <div className="mt-6 ">
              <button className="border w-full border-spacing-2 border-black rounded-xl px-4 py-1 text-center text-md font-bold">
                {" "}
                Place a Bid{" "}
              </button>
            </div>
